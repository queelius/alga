...
namespace algebraic_parsers
{
    ...
    template <typename T, typename ... Ts>
    struct is_stem_variadic
    {
        static const bool value = is_stem<T>::value && is_stem_variadic<Ts...>::value;
    };
    ...
    struct porter2_stemmer
    {
        ...
        // Static function interface for compatibility
        static string stem(string const& input) {
            porter2_stemmer stemmer;
            auto result = stemmer(input);
            if (result) {
                return (string)(*result);
            }
            return input; // Return original if stemming fails
        }
    };
